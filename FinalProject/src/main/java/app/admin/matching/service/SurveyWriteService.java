...
public class SurveyWriteService {
	...
	public String execute(SurveyCommand surveyCommand, Model model, HttpSession session) throws Exception {
		...
		list=surveyCommand.getSurHobby();
		if (list!=null) {
			String [] surHobby= (String[]) list.toArray(new String[list.size()]);
			String hobbySum="";
			for (String s : surHobby) {
				hobbySum+=s+"`";
			}
			surveyDTO.setSurHobby(hobbySum);
		}else {
